function applystyles(styles: any[],text: string){
    let stylestr = styles.map(s => `${s.name}:${s.value}`).join("; ");

    document.writeln(`<p style="${stylestr}">${text}</p>`);
}
applystyles([
...
function prints(temp: any, ...args: any) {
    let result = "";

    for (let i = 0; i < temp.length; i++) {
        if (temp[i] === "%") {
            let index = Number(temp[i + 1]);
            result += args[index - 1];
            i++;
        } else {
            result += temp[i];
